...
const Favorite = () => {
    ...
    function filter() {
        if (catsfilter.length === 0 && brandsfilter.length === 0) {
            setProductsDataFiltered({ ...productsDataFiltered, products: productsData.products.filter((product) => (product.product.name.includes(search))) });
        }
        else if (catsfilter.length === 0) {
            setProductsDataFiltered({ ...productsDataFiltered, products: (productsData.products.filter((product) => (brandsfilter.includes(product.product.brand._id) && product.product.name.includes(search)))) })
        }
        else if (brandsfilter.length === 0) {
            setProductsDataFiltered({ ...productsDataFiltered, products: (productsData.products.filter((product) => catsfilter.includes(product.product.category._id) && (product.product.name.includes(search)))) })
        } else {
            setProductsDataFiltered({ ...productsDataFiltered, products: (productsData.products.filter((product) => (catsfilter.includes(product.product.category._id) && brandsfilter.includes(product.product.brand._id) && (product.product.name.includes(search))))) })
        }
    }
